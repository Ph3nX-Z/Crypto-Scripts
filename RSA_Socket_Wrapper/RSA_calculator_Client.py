...
print(cert)
'''encrypted = test.encrypt("test",cert)
print(encrypted)
...
s.send(cert.encode())
distant_cert = s.recv(buffer).decode()
print(distant_cert)

while True:

    to_send = input("Your Message : ")
    encrypted = test.encrypt(to_send,distant_cert)
    s.send(encrypted)
    message = s.recv(buffer)
    decrypted = test.decrypt(message)
    print(f"From {host}>>{decrypted}")
